...
Un boxplot se construye a partir de los percentiles:
  - Contiene un rectangulo construido en base al 1er y 3er Cuartil (Percentiles 25 y 75) -> su altura es RIC (Q3-Q1).
  - Al rectangulo lo divide horizontalmente una linea, correspondiente a la mediana (Percentil 50 o 2do Cuartil)
  - A partir de cada extremo del rectangulo (superior e inferior) nace una recta o brazo de un determinado largo: El brazo inferior es de largo Li= Q1 - 1.5RIC, y el superior Ls= Q3 + 1.5RIC.
  - Se definen los valores fuera de los brazos (mas externos) como atipicos u outliers.
El largo de los brazos y el criterio para identificar outliers se basa en el comportamiento de una distribucion normal.
...
ggplot(data_tf, aes(region, localidad)) +
...
```{r}
# RESPUESTA
boxplot(diamonds[,c(5,6,8,9,10)], main="Inconsistencias en medidas fisicas")
colSums(is.na(diamonds[,c(5,6,8,9,10)]))

```
  R: Dado los resultados expuestos, dentro de las medidas fisicas no existen valores inexistentes (la suma de los na por columna es 0), pero si valores inconsistentes u outliers (Representados en los puntos alejados del boxplot para cada atributo). Dado la pequeña cantidad de outliers estos pueden ser perfectamente "recortados" manteniendo la representatividad del dataset.
