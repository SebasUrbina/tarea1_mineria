...
El dataset posee 328 filas y 113 columnas. Hicimos uso de la función dim().
...
Cada linea del dataset corresponde a una instancia, que en este caso corresponden a encuentros locales en ciertas localidades del país. Cada una de esta instancia posee como atributos la localidad donde se realizó el cabildo o encuentro y diferentes conceptos constitucionales, los cuales están contabilizados sobre cuantas veces se mencionaron en cada encuentro.
...
Utilizamos la función unique() para encontrar los valóres únicos, y con dim(), podemos obtener la cantidad de valores únicos. Se puede notar que el dataset posee 328 filas únicas, que corresponden a la cantidad de filas que posee el dataset. Por lo tanto no existen localidades repetidas.
...
names(sort(c,decreasing = TRUE))[1:10]
...
columnas_1 <- colSums(atacama[,-c(1,2)])
atacama_2 <- data.frame(concepto=names(atacama[,-c(1,2)]), suma=columnas_1)
...
ggplot(atacama_2, aes(x = reorder(concepto, suma), y = suma)) +
          ...
          coord_flip()+
          ggtitle(label = "Top-10 conceptos región de Atacama")+
          labs(x="Frecuencia", y="Concepto")+theme(plot.title = element_text(hjust = 3))
...
coquimbo <- data_tf %>% filter(region=='Coquimbo')
columnas_2 <- colSums(coquimbo[,-c(1,2)])
coquimbo_2 <- data.frame(concepto=names(coquimbo[,-c(1,2)]), suma=columnas_2)
coquimbo_2 <- coquimbo_2 %>% arrange(-suma)
coquimbo_2 <- coquimbo_2[1:10,]

library(ggplot2)

ggplot(coquimbo_2, aes(x = reorder(concepto, suma), y = suma)) +
          geom_bar(stat = "identity") + 
          coord_flip()+
          ggtitle(label = "Top-10 conceptos región de Coquimbo")+
          labs(x="Frecuencia", y="Concepto")+theme(plot.title = element_text(hjust = 3))
...
metropolitana <- data_tf %>% filter(region=='Metropolitana de Santiago')
columnas_3 <- colSums(metropolitana[,-c(1,2)])
metropolitana_2 <- data.frame(concepto=names(metropolitana[,-c(1,2)]), suma=columnas_3)
metropolitana_2 <- metropolitana_2 %>% arrange(-suma)
metropolitana_2 <- metropolitana_2[1:10,]

library(ggplot2)

ggplot(metropolitana_2, aes(x = reorder(concepto, suma), y = suma)) +
          geom_bar(stat = "identity") + 
          coord_flip()+
          ggtitle(label = "Top-10 conceptos región Metropolitana")+
          labs(x="Frecuencia", y="Concepto")+theme(plot.title = element_text(hjust = 3))
...
Se puede notar que el precio está bastante correlacionado positivamente con las variables carat (quilates), x (largo), y (ancho) y z (profundidad) de los diamantes, pues posee valores muy cercanos a 1, lo cual tiene bastante sentido, ya que un diamante mayores dimensiones tendrá mayor valor.
