...
Se puede observar que los accidentes por colisión presentan la mayor variabilidad por región. También se tiene valores outliers que distorsionan el boxplot. Asimismo se puede notar que los accidentes por Caida y Otros presentan muy poca variabilidad y su mediana es menor respecto a los otros tipos de accidentes.

Se podría realizar un histograma de accidentes por región diferenciados en el tipo de accidente, para tener una mejor idea sobre cuál fue la región con más accidentes y de qué tipo. También se podrían listar las comunas con más y menos accidentes y realizar boxplot y encontrar diferencias. Esto nos ayudaría a comprender en qué región/comuna se produjo la mayor/menor cantidad de accidentes y de qué tipo, y de esta forma direccionar y enfocar políticas que busquen reducir los accidentes.
...
```
R: Se puede notar que el precio está bastante correlacionado positivamente con los atributos físicos(carat(peso),x,y,z) de los diamantes, pues posee valores muy cercanos a 1, lo cual tiene bastante sentido, ya que un diamante más grande tendrá mayor valor.

La correlación más alta para table corresponde al largo(x) del diamante, con un valor de 0.1953, lo que corresponde a una correlación positiva.
